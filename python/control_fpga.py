...
####################################################################################################
	...
	def readline(self):
		...
		response = response.decode('utf-8') #added by Qamesh because replace method is applicable to strings, not bytes.
	...
	def worker(self,loop = True,n = ""):
		##
		# function to execute commands listed on queue
		# loop = Flag to loop execution (default = True)

		statuscolor("Worker for FPGA {:d} started ({:s})".format(self.fpgaid,n),self.fpgaid)
		while(True):
			cmd = self.queue.get() # blocking queue read
			if self.debug:
				debugmessage("Execute queued command at {:d}: {:s}".format(self.queue.qsize(),cmd))
			try:
				self.ser.write(cmd.encode('utf-8'))#Added by Qamesh  to convert the Unicode string to bytes using the UTF-8 encoding.
			except:
				pass
...
#####################################################################################################
	...
	def read_adc(self, chname):
		...
		if chname in self.adcchannels.keys():
			...
			self.worker(loop=False,n= chname)
			resp = self.readline()
			if (self.debug):
				debugmessage(resp)
			res = parse("ADC: Slave {} Channel {} Voltage {}", resp)
	...
	def scb_read_adc(self,pspp,channel):
		...
		self.worker(loop=False,n="SCB reg")
		self.WaitForReady()
