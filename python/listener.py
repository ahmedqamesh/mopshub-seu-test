...
class listener():
	...
	datadir = ''
	filenameVOLTS = '/SEU_Hold_Test_Voltages.txt'
	filenameVOLTSHEX = '/SEU_Hold_Test_VoltagesHex.txt'
	filenamePSPPMON = '/SEU_Hold_Test_PSPPmonitoring.txt'
	filenamePSPPREGS = '/SEU_Hold_Test_PSPPregisters.txt'
	filenameDATA_IN = '/SEU_Hold_Test_DataIN.txt'
	filenameDATA_S = '/SEU_Hold_Test_DataSIMPLE.txt'
	filenameDATA_TMR = '/SEU_Hold_Test_DataTMR.txt'
	filenameINFO = '/SEU_Hold_Test_Info.txt'
	filenameDIG = '/SEU_Hold_Test_DigitalSignals.txt'
	...
	def __init__(self, fpgaid, baudrate=921600, DEBUG=False):
	...
	def init_holdtest(self,chipID,periode_holdshift,shift_frequency,periode_adcmon,periode_psppmonvalues,periode_psppregisters,periode_psppwrite):
		...
		datafile.write("PSPP monitoring interval: {:.3f}s\tPSPP register interval: {:.3f}"\
				"PSPP write interval:{:.3f}\r\n".format(\
				periode_psppmonvalues,periode_psppregisters,periode_psppwrite))
		
		datafile.write("Time[s]\t#SEU_s\t#SEU_s 0to1\t#SEU_s 1to0\t#SEU_t\t#SEU_t 0to1\t#SEU_t 1to0\tSCB error\tComment\r\n")
		datafile.close()
	...
	def write_info(self,timestamp,s_nb,s_high,s_low,t_nb,t_high,t_low,scberr,comment):
		...
		# @param scberr = SCB error occured
		...
		while(1):
			...
			# read PSPP registers
				...
				if (len(infostring) < 25):
					warning("FPGA{:d} ".format(self.fpgaid)+"SCB Communication error")
					self.write_info(line['timestamp'],self.simple_seus,self.simple_high,self.simple_low,
							self.tmr_seus,self.tmr_high,self.tmr_low,
							'1','SCB read all registers failed. Not enough data')
				else:
			...
			# read PSPP monitoring values
				...
				infostring = line['line'].split(' ')
				if (len(infostring) < 6):
					warning("FPGA{:d} ".format(self.fpgaid)+"SCB Communication errror")
					self.write_info(line['timestamp'],self.simple_seus,self.simple_high,self.simple_low,
							self.tmr_seus,self.tmr_high,self.tmr_low,
							'1','SCB read monitoring failed. Not enough data')
				else:
					info = [int(x,16) for x in infostring[2:]]
					self.write_pspp_monitoring(line['timestamp'],info)
					# print PSPP monitoring info on terminal
					statuscolor("FPGA{:d} ".format(self.fpgaid)+\
						"PSPP Monitoring Data:\tVmod\t= {:03X}\t\tTemp0\t= {:03X}\t\tTemp1\t= {:03X}\t\tBypass\t= {:02X}"\
			...
			# read response from PSPP register write command
				...
				if (len(infostring) < 4):
					...
					self.write_info(line['timestamp'],self.simple_seus,self.simple_high,self.simple_low,\
							self.tmr_seus,self.tmr_high,self.tmr_low,\
							'1','SCB write register failed. Not enough data')
				else:
					self.write_info(line['timestamp'],self.simple_seus,self.simple_high,self.simple_low,\
							self.tmr_seus,self.tmr_high,self.tmr_low,\
							'0',"SCB write register: {:s} with {:s}"\
			...
			# read chip monitoring values from ADC
				...
				valstr = line['line'].split(' ')
				if (len(valstr) < 16):
					warning("FPGA{:d} ".format(self.fpgaid)+"ADC Communication errror")
					self.write_info(line['timestamp'],self.simple_seus,self.simple_high,self.simple_low,\
							self.tmr_seus,self.tmr_high,self.tmr_low,\
