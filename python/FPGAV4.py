...
while(running):
	...
	if selection == '0':
		...
		try:
			channel = int(raw_input('DAC-Channel (1-8): '))
			
			value = int(float(raw_input('Value in  V: '))*1000)
			command = reg.DAC(channel, value)
			fpga.write( command )
			tmp = fpga.readline()
		...
		try:
			addr = raw_input('Chip-address(Hex): ')
		...
		try:
			addr = raw_input('Chip-address(Hex): ')
			register = raw_input('Which register(Hex): ')
			data = raw_input('What data (Hex, 1byte): ')
		...
		operation_mode=raw_input("Operation Mode: ")
		...
		addr = raw_input("Chip address: ")
		adc = raw_input("Which adc(0-7): ")
		value = test.SingleChipADC(adc, addr, fpga)
		print("Value of ADC"+adc+": "+str(value))

	elif (selection == '14'):
		addr = raw_input("Chip address: ")
		register = raw_input("Which register(0-12): ")
