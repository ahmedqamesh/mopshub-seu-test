...
if __name__ == '__main__':
	...
	l = [listener.listener(fpgaid=n,baudrate=baudrate,DEBUG=debug) for n in range(numfpga)]
	# open a second serial port to write commands
	f = [fpga_control.fpga(fpgaid=n,baudrate=baudrate,DEBUG=debug) for n in range(numfpga)]

	# print settings and ask for confirmation
	status("\n\t --------------------------------------------------------------------")
	status("Irradiation Test for PATT")
	status("--------------------------------------------------------------------")
